    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", width: "100%" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <span style={{ padding: "4px 12px", borderRadius: 20, fontSize: 11, fontFamily: FONTS.heading, fontWeight: 700, background: `${cat.color}cc`, color: "#fff", display: "inline-flex", alignItems: "center", gap: 4 }}><Icon name={cat.icon} size={12} /> {cat.label}</span>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 24px 60px", display: "grid", gridTemplateColumns: "1fr 290px", gap: 40, alignItems: "start" }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                {item.fileUrl && (
                  <a href={item.fileUrl} target="_blank" rel="noreferrer"
                    ...
                    <Icon name="Download" size={14} /> {item.fileLabel || "Baixar Documento"}
                  </a>
                )}
                {item.externalLink && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, position: "sticky", top: 80 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: "18px 16px", boxShadow: "0 2px 14px rgba(0,0,0,0.07)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div>
                ...
                <span style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 10px", borderRadius: 20, fontSize: 12, fontFamily: FONTS.heading, fontWeight: 700, background: `${cat.color}15`, color: cat.color }}><Icon name={cat.icon} size={12} /> {cat.label}</span>
              </div>
              {item.publishedAt && (
                <div>
              ...
              {(item.fileUrl || item.externalLink) && (
                <div>
                  ...
                  {item.fileUrl && <a href={item.fileUrl} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 10px", borderRadius: 7, background: `${COLORS.primary}10`, color: COLORS.primary, textDecoration: "none", fontFamily: FONTS.heading, fontSize: 12, fontWeight: 700, marginBottom: 6 }}><Icon name="Download" size={12} /> {item.fileLabel || "Baixar"}</a>}
                  {item.externalLink && <a href={item.externalLink} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 10px", borderRadius: 7, background: COLORS.grayLight, color: COLORS.dark, textDecoration: "none", fontFamily: FONTS.heading, fontSize: 12, fontWeight: 700 }}><Icon name="ExternalLink" size={12} /> Link Externo</a>}
                </div>
              )}
            </div>
          </div>
          ...
          {related.length > 0 && (
            <div style={{ background: "#fff", borderRadius: 12, padding: "18px 16px", boxShadow: "0 2px 14px rgba(0,0,0,0.07)" }}>
              <h3 style={{ fontFamily: FONTS.heading, fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: 1.5, color: COLORS.dark, margin: "0 0 12px", paddingBottom: 8, borderBottom: `1px solid ${COLORS.grayLight}`, display: "flex", alignItems: "center", gap: 4 }}><Icon name={cat.icon} size={12} /> Outros em "{cat.label}"</h3>
              {related.map(r => (
          ...
          <Link to="/intranet" style={{ display: "block", textAlign: "center", padding: "10px 0", borderRadius: 8, background: COLORS.dark, color: "#fff", textDecoration: "none", fontFamily: FONTS.heading, fontSize: 13, fontWeight: 700 }}>
            <Icon name="Lock" size={13} /> Acessar Intranet
          </Link>
        </div>
      </div>
    </div>
