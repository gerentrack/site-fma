/**
 * VerificarEmailIntranet.jsx
 * Verificação de e-mail do árbitro após login (código de 6 dígitos).
...
export default function VerificarEmailIntranet() {
  const { session, logout } = useIntranet();
  ...
  const verificar = async () => {
    ...
    setVerifying(true);
    await RefereesService.update(session.refereeId, {
      emailVerified: true,
  ...
  const salvarNovoEmail = async () => {
    ...
    setSavingEmail(true);
    const r = await intranetAuthAPI.updateEmail(session.refereeId, email, password);
    setSavingEmail(false);
    if (r.error) { setEmailError(r.error); return; }
    // Reload para atualizar session com novo email
    ...
    <div style={{ minHeight: "100vh", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: "48px 40px", width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: FONTS.heading, fontWeight: 800, fontSize: 22, color: COLORS.dark, margin: 0 }}>Verificar E-mail</h1>
          <p style={{ fontFamily: FONTS.body, fontSize: 13, color: COLORS.gray, marginTop: 8 }}>
            Precisamos confirmar seu e-mail <strong>{currentEmail}</strong> antes de continuar.
          </p>
        </div>
