...
export default function HeroBanner({ banners = [] }) {
  ...
  return (
    <div style={{ background: b.bg, minHeight: 340, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", textAlign: "center", padding: "60px 24px", position: "relative", overflow: "hidden", transition: "background 0.8s ease" }}>
      ...
      <div style={{ fontSize: 72, marginBottom: 16, filter: "drop-shadow(0 4px 12px rgba(0,0,0,0.3))" }}>{b.icon}</div>
      <h1 style={{ fontFamily: FONTS.heading, fontSize: "clamp(2rem, 5vw, 3.5rem)", fontWeight: 800, color: COLORS.white, margin: "0 0 12px", textTransform: "uppercase", letterSpacing: 2, textShadow: "0 2px 16px rgba(0,0,0,0.3)" }}>{b.title}</h1>
      <p style={{ fontFamily: FONTS.body, fontSize: "clamp(1rem, 2vw, 1.2rem)", color: "rgba(255,255,255,0.9)", maxWidth: 560, margin: "0 0 28px", lineHeight: 1.5 }}>{b.subtitle}</p>
      <a href={b.ctaLink || "#"} style={{ background: COLORS.white, color: COLORS.primaryDark, padding: "13px 34px", borderRadius: 40, fontWeight: 700, fontFamily: FONTS.heading, textDecoration: "none", fontSize: 15, letterSpacing: 1, textTransform: "uppercase", boxShadow: "0 4px 20px rgba(0,0,0,0.2)", transition: "transform 0.2s, box-shadow 0.2s" }}
